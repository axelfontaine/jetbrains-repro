...
@Serializable
data class Location(val lat: Double, val lng: Double, val accuracy: Float, val timestampUTC: Date = Date()) {
    init {
        if (lat < -90 || lat > 90) throw IllegalArgumentException("Invalid latitude: $lat")
        if (lng < -180 || lng > 180) throw IllegalArgumentException("Invalid longitude: $lng")
        if (accuracy < 0) throw IllegalArgumentException("Invalid accuracy: $accuracy")
    }

    /**
     * The distance in meters from this location to this other location.
     * Elevation differences are not taken into account.
     */
    ...
    fun distanceTo(loc: Location): Double {
        ...
        val lat2 = loc.lat * (PI / 180.0)
        val lon1 = lng * (PI / 180.0)
        val lon2 = loc.lng * (PI / 180.0)
